...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (!PlayStateChangeables.Optimize)
		{
			switch (stageCheck)
			{
				case 'rooftops':
					{
						defaultCamZoom = 0.75; //J: in order for this to look good, this is necessary
						curStage = 'rooftops';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('rooftops/rooftops'));
		...
		//J: STUPID!!!!!!! BAD!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		//J: IF THERE IS NO GF FOUND IN THE CHART, THEN THAT IS THE FUCKING CHARTER ISSUE DOWN TO THE SOURCE CODE!!!!!!!!!!!!!!!
		//J: I DON'T KNOW WHAT THE FUCK WOULD'VE CAUSED THAT, BUT THIS IS UNNECESSSARY!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		//J: I AM SO UNREASONABLY ANGRY!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		switch (SONG.player2)
		{
			...
			case 'dracobot':
				dad.y += 150;
			case 'angyDracobot':
				if (songLowercase == "dragon-horns"){
					var evilTrail = new FlxTrail(dad, null, 4, 24, 0.3, 0.069);
					// evilTrail.changeValuesEnabled(false, false, false, false);
					// evilTrail.changeGraphic()
					add(evilTrail);
				}
				if (songLowercase == "dragonsnaps"){
					var evilTrail = new FlxTrail(dad, null, 4, 24, 0.3, 0.069);
					// evilTrail.changeValuesEnabled(false, false, false, false);
					// evilTrail.changeGraphic()
					add(evilTrail);
		...
		switch (curStage)
		{
			...
			case 'schoolEvil':
				...
				gf.y += 300;
			case 'rooftops':
				dad.x -= 120; //J: should just change the idle but this should work
				dad.y += 30;
				boyfriend.x += 250; //J: makes it look less stupid to me lol
				boyfriend.y += 15;
				gf.y -= 65;
		}
		...
		if (isStoryMode)
		{
			switch (StringTools.replace(curSong, " ", "-").toLowerCase())
			{
				...
				case 'senpai':
					// schoolIntro(doof); //J: (old shit, very bad)
					runDialogue(doof);
				case 'roses':
	...
	{ // this actually handles press inputs
		...
		if (keys[data])
		{
			...
			//J: note to kade just in case he ever looks at this code: using traces slaughters the FPS
			//(especially with loops that are active during gameplay)
			//J: in a rhythm game where every millisecond counts, this is really bad
			//J: don't do this
			return;
		}
