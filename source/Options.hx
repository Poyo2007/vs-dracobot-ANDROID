...
class OptionCategory //J: all options are handled here in Options.hx -- if you want an option, you go here *first*
{
...
class DFJKOption extends Option //J: the class names are supposed to be kind of self-explanatory, such as DFJK option here for, well, DFJK input
{
	private var controls:Controls; //(i have a dual monitor setup (yay) so i can see when you chat)

	public function new(controls:Controls) //J: haxeflixel, afaik, is built off of C# or Java, either one, based on private/public instruction
	{
		super(); //J: depending on experience with GM8 code, it can either be really freeing or really limiting, and it depends on what you make
		this.controls = controls; //J: HaxeFlixel code is far more freeing due to the high difficulty ceiling, and, were Kade to consistently add comments,
	} //J: the learning curve wouldn't be so harsh either
...
class WarningOption extends Option
{
	...
	public override function press():Bool
	{
		FlxG.save.data.warningScreen = !FlxG.save.data.warningScreen;
		display = updateDisplay();
		return true;
	}
	...
	public override function updateDisplay():String
	{
		return FlxG.save.data.warningScreen ? 'Warning Screen On' : 'Warning Screen Off';
	}
}
...
class Fullscreen extends Option
{
	...
	public override function press():Bool
	{
		...
		trace(FlxG.save.data.fullScreen + "YES FULLSCREEN");
		display = updateDisplay();
		return true;
	}
	public override function updateDisplay():String
	{
		return FlxG.save.data.fullScreen ? "Fullscreen" : "Windowed";
	}
}
...
class ThornsHorns extends Option{
	public function new(desc:String)
		{
			super();
			description = desc;
		} //J: all of these follow a simple formula, so it's a good idea just to follow them

		public override function press():Bool
		{
				FlxG.save.data.thornshorns = !FlxG.save.data.thornshorns; //(that's what this is here -- it changes this to be the opposite, aka, true to false, false to true)
				display = updateDisplay();
				return true;
		} //J: yes! the save data is handled in KadeEngineData.hx, which changes when toggled

		private override function updateDisplay():String
		{		
				return "Week 6 Final Song " + (FlxG.save.data.thornshorns ? "Dragon Horns" : "Thorns"); //J: i'm not 100% sure on how this operates tho lol
		} //J: you need to go into KadeEngineData to change this from what would be "null" to a value -- such as true, or false when making save edits like this
}