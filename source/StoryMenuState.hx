...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		//your welcome for me keeping the easter egg across ports lol.
		if (roundedNum == 12 && stupidShit == false || roundedNum == 64 && stupidShit == false) {
			// for (i in 0...10){
				// trace("J: HOLY SHIT!!");
				// trace("J: " + roundedNum);
			// }
			...
			stupidShit = false;
			// trace("J: not based :[");
			// trace("J: old: " + roundedNum);
			roundedNum = Std.int(Math.random() * 64);
			// trace("J: next: " + roundedNum);
		}
	...
	function changeWeek(change:Int = 0):Void
	{
		...
		if (txtWeekTitle.text == "DRACOBOT") { //J: any fellow programmers looking at this: this is a reference to vs. Jacobe, one of my fellow programmer buds
			if (holyShit == true && stupidShit == false){ //J: should i add this to V2? yes. should i add this to Dracobot? questionable.
				// trace("J: WOOOOOOOOOOOOOOOOOOOOOOO");
				txtWeekTitle.text = "ERROR 404: WEEK NOT FOUND";
				// trace(txtWeekTitle.text);
			} else {
				// trace("J: wait waht the fuck???");

				//J: do nada :]
			}
		} if (txtWeekTitle.text == "ERROR 404: WEEK NOT FOUND" && holyShit == true && stupidShit == false) { //J: yeah yeah this looks like shit but
			if (roundedNum != 12 && roundedNum == 64) { //J: it makes sense to me and i've made it as simple as possible without making me look like YanDev
				//J: nada -- they gucci
			}
			if (roundedNum == 12 && roundedNum != 64) {
				//J: nada -- still gucci
			}
			if (roundedNum != 12 && roundedNum != 64) {
				txtWeekTitle.text = "DRACOBOT";
				holyShit = false;
			}
		}
