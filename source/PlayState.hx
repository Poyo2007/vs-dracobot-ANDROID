...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'fields':
				...
				var ChillyFields = new FlxSprite(-380, -200 + 800).loadGraphic(Paths.image('Chilly_Fields_BG_foreground'));
				var ChillyTrees = new FlxSprite(-300, -200).loadGraphic(Paths.image('fields_trees'));
				...
				var DracobotOnRockLol = new Character(1240, 275, 'dracorock');
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void //tried to fix, am suffering a bit probably just recode lol
	{
		...
		{
			blackScreen = new FlxSprite(-250, -250).makeGraphic(Std.int(FlxG.width * 2), Std.int(FlxG.height * 2), 0xFF000000);
			blackScreen.scrollFactor.set();
			blackScreen.alpha = 1;
			add(blackScreen);
			...
			{ // 1
				...
				{ // 2
					...
					{ // 3
						...
						// { //4
						add(dialogueBox);
						trace('Dialog box all seems in order');
						// } //4
					} // 3
					else
					{ // 3
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
				{
					...
					if(daNote.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
						...
						if(daNote.noteType == 'GF Sing') {
							...
							if (ClientPrefs.arroCamMove && Math.floor(curStep / 16) > -1 && !SONG.notes[Math.floor(curStep / 16)].mustHitSection && !isCameraOnForcedPos)
								ArrowCamThing(gf);
							gf.holdTimer = 0;
						} else {
